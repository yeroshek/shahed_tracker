...
if __name__ == "__main__":
    with multiprocessing.Manager() as manager:
        ...
        frame_manager_process = multiprocessing.Process(target=frame_manager.capture_frames, args=(2, frames))
        frame_manager_process.start()

        # Start video capture process
        video_capture_process = multiprocessing.Process(target=video_capture.capture_video, args=(frames, 2))
        video_capture_process.start()

        # Start video streaming process
        app = video_streaming.video_streaming(2, frames)
