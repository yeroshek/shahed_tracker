...
def read_frame(cap):
    ret, frame = cap.read()
    if not ret:
        return  # If no frame is captured, skip the rest of the loop
...
@app.route('/scan')
def scan():
    return Response(scan_air_route(), mimetype='multipart/x-mixed-replace; boundary=frame')
